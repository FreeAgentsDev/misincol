...
export function PlansListClient({ plans }: Props) {
  const [selectedPlan, setSelectedPlan] = useState<PlanWithTeam | null>(null);
  const [selectedTeam, setSelectedTeam] = useState<{ id: string; name: string; leader: string } | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Agrupar planes por estado
  const plansByStatus: Record<PlanStatus, PlanWithTeam[]> = {
  ...
  const handlePlanClick = (plan: PlanWithTeam) => {
    setSelectedPlan(plan);
    setSelectedTeam({
      id: plan.teamId,
      name: plan.teamName,
      leader: plan.teamLeader
    });
    setIsModalOpen(true);
  };
